...
#
# Edit by Louis Brunner (2015-09-28 - VirtualBusLogicController -> VirtualLsiLogicController)
# Edit by Louis Brunner (2015-09-29 - Rework the NIC creation)
...
sub get_network {
   ...
   if($network_name) {
      ...
      foreach my $dvs (@dvs_list) {
         if($network_name eq $dvs->{portgroupName}) {
            $network = $dvs;

	    my $nic_port_info =
	       DistributedVirtualSwitchPortConnection->new(switchUuid => $network->{switchUuid}, portgroupKey => $network->{portgroupKey});
