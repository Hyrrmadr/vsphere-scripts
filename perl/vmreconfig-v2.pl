...
#
# Edit by Louis Brunner (2015-09-29 - Make NIC management less broken)
...
# get a list of distributed virtual switches
# ==========================================
sub get_dvs_list {
   my $rp_name = "Hosting"; # Yes, I know, its hardcoded, should I have to access so many classes to get a DVS list? :/

   my @dvs_list = ();

   my $rp_views = Vim::find_entity_views(view_type => "ResourcePool", filter => {name => $rp_name});
   my $rp_view = shift (@$rp_views);
   if ($rp_view) {

      my $cr_view = Vim::get_view(mo_ref => $rp_view->{owner});
      if ($cr_view) {

         my $eb_view = Vim::get_view(mo_ref => $cr_view->{environmentBrowser});
         if ($eb_view) {

            my $config = $eb_view->QueryConfigTarget();

            if ($config && $config->{distributedVirtualPortgroup}) {

               my $dvs_info = $config->{distributedVirtualPortgroup};

               foreach my $dvs (@$dvs_info) {
...
sub get_dvs_by_portgroup_name {
   my ($network) = @_;

   my @dvs_list = get_dvs_list();
   foreach my $dvs (@dvs_list) {
      if($network eq $dvs->{portgroupName}) {
         return $dvs;
      }
   }
...
# Most of this subroutine is lifted from VMUtil.pm create_network_spec
# and its helper functions
# ====================================================================
sub my_create_network_spec {
   my ($vm_view, $network, $operation) = @_;
   ...
   if($operation eq 'add') {
      ...
      my $dvs = get_dvs_by_portgroup_name($network);

      if ($dvs) {
	  my $nic_port_info =
          	DistributedVirtualSwitchPortConnection->new(switchUuid => $dvs->{switchUuid}, portgroupKey => $dvs->{portgroupKey});

          my $nic_backing_info =
                VirtualEthernetCardDistributedVirtualPortBackingInfo->new(port => $nic_port_info);

          my $vd_connect_info =
                VirtualDeviceConnectInfo->new(allowGuestControl => 0,
                                              connected => 0,
                                              startConnected => 1);

          $device = VirtualE1000->new(backing => $nic_backing_info,
                                      key => 2,
                                      unitNumber => 2,
      ...
      if ($dvs) {
       ...
       foreach my $device_i (@$devices) {
          if (!defined $device_i->backing || ref($device_i->backing) ne 'VirtualEthernetCardDistributedVirtualPortBackingInfo') {
	     next;
          }
          if ($device_i->backing->port->switchUuid eq $dvs->{switchUuid} && $device_i->backing->port->portgroupKey eq $dvs->{portgroupKey}) {
             $device = $device_i;
             last;
          }
       }
	 ...
	 Util::trace(0,"\nRemoving NIC with the name '" . $network . "' . . .");
         $device->{deviceInfo}->{label} = "Adaptateur";
      }
   }
...
sub get_device_config_specs {
   ...
   if($reconfig_hash{'Add-Device'}{'Network-Adapter'}{'Name'}) {
      my $device_spec
         = my_create_network_spec($vm_view,
              $reconfig_hash{'Add-Device'}{'Network-Adapter'}{'Name'}, 'add');
      if ($device_spec) {
   ...
   if($reconfig_hash{'Remove-Device'}{'Network-Adapter'}{'Name'}) {
      my $device_spec
         = my_create_network_spec($vm_view,
              $reconfig_hash{'Remove-Device'}{'Network-Adapter'}{'Name'}, 'remove');
      if ($device_spec) {
         @device_config_specs = (@device_config_specs, $device_spec);
      }
   }
